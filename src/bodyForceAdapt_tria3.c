...
#include "constants.h"
#include "bodyForceAdapt_tria3.h"
                         ...
                         evalNode *integralNodes)         
{
                ...
                if( (integralNodes[i].Esq*integralNodes[i].vol/average) > localError ){
                    if((size + newNodes + 8) < MAXNODES){ // subdivide this cell
                        ...
                        for(j = currentSize; j <= last; j++){
                            ...
                            field_tria3(Xin,mDomainNodes,mDomainElems,
                            vDomainSolution,E);
    ...
    for(i = 0 ; i < nNodes; i++){
        ...
        Xin[2] = mNodes[i][2];
        bc = vBCType[i];
        if(bc == 0 || bc == 6) getNormal_tria3(i+1,mNodes,mElems,normal);               
