...
#include "constants.h"
#include "field_tria3.h"

int field_tria3(double *Xin, double **mNodes, unsigned int **mElems, double *vB,
                double *mE)
{
	const unsigned int ELEMS = nDomainElems, DIM = 3, FLAG = 1, NODES_IN_ELEM = 3;
	unsigned int i, j, k, currentNode, SinNode, test;
	double A, B, dx, dy, dz, normal[3], SubE[9], X[3][3];
	...
	for(i = 0; i < ELEMS; i++)
	{
	   	...
	   	{
			X[j][0] = mNodes[mElems[i][j]-1][0];
			X[j][1] = mNodes[mElems[i][j]-1][1];
			X[j][2] = mNodes[mElems[i][j]-1][2];
   		}
   		...
   		if(test == 0) intH_tria3(FLAG,X,Xin,SubE,normal);
   		else intSingularH_tria3(FLAG,SinNode,X,Xin,SubE,normal);
