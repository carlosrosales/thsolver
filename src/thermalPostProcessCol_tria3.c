/******************************************************************************
* File      : thermalPostProcessCol_tria3.c                                   *
...
* points within the specified nColumns from the calculation.                  *
...
#include "constants.h"
#include "thermalPostProcessCol_tria3.h"
...
{
	FILE   *fT;
	unsigned int count, i, j, k, m, nxi, nyi, nzi, Tcount;
		...
		for(j = 0; j < NT; j++){
			...
			for(k = 0; k < NT; k++){
				...
				for(m = 0; m < nColumns; m++){
		   			if(nColumnType == 1){
				        if(sqrt(pow(Xin[0] - xColumns[m][0],2.0) + pow(Xin[1] - xColumns[m][1],2.0)) > xColumns[m][2]) count++;
			        }
			        else{
				        if((fabs(Xin[0] - xColumns[m][0])> xColumns[m][2]) && (fabs(Xin[1] - xColumns[m][1]) > xColumns[m][2])) count++;
			        }
	   			}
	   			if(count == nColumns){
					temperature_tria3(nInterp,b0,Xin,mNodes,mElems,vB,xi,yi,zi,bi,&T);
