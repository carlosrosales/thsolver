...
* is calculated using linear elements in    5-----6 |                         *
* the boundary (3-noded triangles) and      | 4---|-3                         *
...
#include "constants.h"
#include "bodyForceCubat_tria3.h"
                         ...
                         double *vDomain, unsigned int *vBCType, double *b)
{
    ...
    for(i = 0; i < nCellNodes; i++){
        ...
        Xin[1] = cellNodes[i][1];
        Xin[2] = cellNodes[i][2];    
        field_tria3(Xin,mDomainNodes,mDomainElems,vDomainSolution,E);
    ...
    for(p = 0 ; p < nNodes; p++){
        ...
        Xin[2] = mNodes[p][2];
        bc = vBCType[p];
        if(bc == 0 || bc == 6) getNormal_tria3(p+1,mNodes,mElems,normal);
