                       ...
                       double *vB, double *vDomain)
{
	...
	unsigned int i, interfaceID, j, k, mat1, mat2, currentNode, SinNode, bc;  
	double B, Kdif, Ktot;
	...
	/* FIND CONTRIBUTIONS TO EACH NODE AND ASSEMBLE */
	for(i = 0; i < nNodes; i++)
	{
    	/* GET EVALUATION NODE */
    	Xeval[0] = mNodes[i][0];
    	...
    	/* OPERATIONS NECESSARY FOR INTERFACES ONLY */
    	if(bc == 0)
    	{
			/* GET INTERFACE AND NORMAL AT EVALUATION NODE */
    	...
    	{	
    		...
    		if(bc == 1)          
    		{
	    		...
	    		/* ASSEMBLE COEFFICIENT MATRIX mA */
	    		for(k = 0; k < NODES_IN_ELEM; k++)
	    		{
					currentNode = mElems[j][k] - 1;
