...
#include "constants.h"
#include "temperatureCubat_tria3.h"

int temperatureCubat_tria3(double *Xin, double **mNodes, unsigned int **mElems,
    double *vB,  double **cellNodes, unsigned int **cells, double *b, double *T)
{
    ...
    double  L[3], N[8], normal[3], SubT[3], W[3], XL[3];
    double  X[3][3], XC[8][3];
    ...
    for(i = 0; i < ELEMS; i++){
        ...
        /* Add contributions to temperature for all j nodes in element i */
        for(j = 0; j < NODES_IN_ELEM; j++)
            Ttmp += SubT[j]*vB[mElems[i][j] - 1];
    }
